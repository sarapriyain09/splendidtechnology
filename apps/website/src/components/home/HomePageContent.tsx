...
const sectionAnchors = [
  ...
  { label: "Videos", href: "#platform-videos" },
...
const crmVideoUrl = "https://youtu.be/4dZbTW34Nk8";
const crmVideoEmbed = "https://www.youtube-nocookie.com/embed/4dZbTW34Nk8?rel=0&modestbranding=1&playsinline=1";
const growthPlatformYoutubeUrl = process.env.NEXT_PUBLIC_GROWTH_PLATFORM_VIDEO_URL ?? "https://youtu.be/qvKj3LYBgt0";
const aimediaYoutubeUrl = process.env.NEXT_PUBLIC_AIMEDIA_STUDIO_VIDEO_URL ?? "https://youtu.be/QqXRLiMvHbk";
...
const platformVideos = [
  {
    ...
    description: "Overview of CRM, Sales, CallCRM, Marketing, Automation, and Analytics in one connected platform.",
    youtubeUrl: growthPlatformYoutubeUrl,
    src: "/videos/growth-platform.mp4",
  },
  {
    title: "AIMedia Studio",
    description: "See how teams generate scripts, voiceovers, podcasts, videos, and avatar content faster.",
    youtubeUrl: aimediaYoutubeUrl,
    src: "/videos/aimedia-studio.mp4",
  },
];
...
export function HomePageContent() {
  ...
  return (
    <div className="relative overflow-hidden bg-[var(--background)]">
      ...
      <motion.section id="platform-videos" variants={sectionFade} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.2 }} className="mx-auto w-full max-w-7xl px-4 py-16 sm:px-6 sm:py-18 lg:px-8">
        <h2 className="text-3xl font-bold text-[#0e1629] sm:text-4xl">Platform Videos</h2>
        <p className="mt-3 max-w-3xl text-sm leading-6 text-[#4a5a7a]">
          Watch quick walkthroughs of both Velynxia pillars.
        </p>
        <div className="mt-8 grid gap-6 lg:grid-cols-2">
          {platformVideos.map((video) => (
            <article key={video.title} className="overflow-hidden rounded-3xl border border-[#dce8ff] bg-white p-3 shadow-[0_16px_42px_rgba(16,39,88,0.14)]">
              <div className="relative aspect-video overflow-hidden rounded-2xl border border-[#dce8ff] bg-[#f8fbff]">
                {toNoCookieEmbed(video.youtubeUrl) ? (
                  <iframe
                    src={toNoCookieEmbed(video.youtubeUrl)}
                  ...
                  <video controls preload="metadata" className="h-full w-full" title={video.title}>
                    <source src={video.src} type="video/mp4" />
                    Your browser does not support the video tag.
                  </video>
                )}
              </div>
              <h3 className="mt-4 text-xl font-bold text-[#122443]">{video.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[#425375]">{video.description}</p>
              {video.youtubeUrl ? (
                <a
                  href={video.youtubeUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="mt-4 inline-flex items-center rounded-md border border-[#cfe0ff] bg-white px-3 py-1.5 text-xs font-semibold text-[#2c4d87] transition hover:bg-[#f4f8ff]"
                >
                  Watch on YouTube
                </a>
              ) : null}
