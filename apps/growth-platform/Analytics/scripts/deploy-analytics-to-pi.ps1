...
$archivePath = Join-Path $repoRoot "analytics-app.tar"
...
tar --exclude=node_modules --exclude=.next --exclude=analytics-app.tar -cf $archivePath .
Assert-LastExit "Create tar archive"

Write-Host "==> Uploading archive to Pi" -ForegroundColor Cyan
scp $archivePath "$PiHost`:/tmp/analytics-app.tar"
...
$remoteCommands = @(
  ...
  "if pm2 describe '$Pm2App' >/dev/null 2>&1; then pm2 restart '$Pm2App' --update-env; else NEXTAUTH_URL=https://analytics.velynxia.com PORT=$Port pm2 start npm --name '$Pm2App' --cwd '$RemoteRoot' -- run start -- -p $Port; fi",
  "pm2 save",
  "pm2 describe '$Pm2App'"
)
...
Write-Host "Analytics deployment complete." -ForegroundColor Green
