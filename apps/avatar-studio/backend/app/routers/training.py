...
from app.workers.tasks.training import start_avatar_training
...
def enqueue_training(training_id: str, db: Session = Depends(get_db)) -> dict[str, str]:
    ...
    start_avatar_training.delay(training_id)
...
def complete_resumable_upload(
    ...
    final_object_name = f"training/{training_id}/{payload.upload_id}/{session['filename']}"
