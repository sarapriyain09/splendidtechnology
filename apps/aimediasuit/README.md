...
Avatar Studio now includes Clone Me workflows for user-specific AI presenter setup and MP4 generation pipelines.
...
- Avatar Studio (Phase 3 in progress: Clone Me)
	- Clone Me wizard with step-based onboarding for photos, training video, and voice clone assets
	- My Clones tab for clone card management (generate, edit, delete)
	- Settings tab for default voice, background, music, intro/outro, and logo preferences
	- Clone project generation pipeline reusing Voice, Background Music, Subtitle, and Video render modules
...
- Step 1: Upload Photos (JPG/PNG, 20-50 files, drag and drop, thumbnail preview, remove image)
- Step 2: Upload Training Video (MP4, recommended 5-10 min, shows duration/resolution/FPS)
- Step 3: Voice Clone (record/upload WAV/MP3, minimum 5 min, recommended 15-30 min, waveform/player preview)
- Step 4: Clone Profile (name, language, accent, speed, gender, background, category)
- Step 5: Training (uploading, processing, face training, voice training, ready)

Clone Me runtime safeguards:

- Photo upload now validates actual image decodeability before persistence; corrupt JPG/PNG payloads are rejected with HTTP 400.
- Photo dimensions above 8192px on either axis are rejected.
- Subtitle burn-in now escapes Windows file paths for FFmpeg filter compatibility.
- Regression tests are available with: npm run test:unit
- Fixture guidance: avoid tiny synthetic PNG byte payloads for end-to-end runs; use valid JPEG/PNG files generated by camera exports or FFmpeg image outputs.

Clone generation screen:
...
- See: ai_gateway_fastapi/README.md
- Switch text provider via env: AI_TEXT_PROVIDER=openai|ollama
- When using ollama set: OLLAMA_GATEWAY_CHAT_URL and OLLAMA_TEXT_MODEL
...
- POST /api/media/clone/photos
- POST /api/media/clone/video
- POST /api/media/clone/voice
- POST /api/media/clone/train
- GET /api/media/clone/history
- DELETE /api/media/clone/[id]
- POST /api/media/clone/generate
...
- app/api/media/clone: Clone Me API handlers
...
- components/avatar-studio: Includes Create, Templates, Clone Me, My Clones, History, Settings tabs
...
- CloneStatus: UPLOADING, PENDING, PROCESSING, TRAINING, READY, FAILED
